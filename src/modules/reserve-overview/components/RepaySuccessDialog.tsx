...
interface RepaySuccessDialogProps {
  open: boolean;
  onClose: () => void;
  /** Human-readable amount repaid */
  amount: string;
...
export function RepaySuccessDialog({ open, onClose, amount, symbol, txHash, explorerUrl }: RepaySuccessDialogProps) {
      ...
      <DialogContent className='sm:max-w-md'>
        <div className='flex flex-col items-center gap-5 pt-4'>
          ...
          <DialogDescription className='text-center text-base text-muted-foreground'>
            {t('youRepaid')} <span className='font-semibold text-foreground'>{formattedAmount}</span>{' '}
