...
import { type BigNumberValue, normalize, valueToBigNumber, valueToZDBigNumber } from '../../bignumber';
...
function getComputedReserveFields({
  ...
  const utilizationRate = totalLiquidity.eq(0) ? '0' : valueToBigNumber(totalDebt).dividedBy(totalLiquidity).toFixed();
 ...
 */
export function formatReserve({ reserve, currentTimestamp }: FormatReserveRequest): FormatReserveResponse {
