...
describe('calculateUserReserveTotals', () => {
  ...
  it('should compute totals from user reserve array', () => {
    ...
    expect(userReserveTotals.totalLiquidityMarketReferenceCurrency.toFixed()).toEqual('11819951652201573862');
    expect(userReserveTotals.totalBorrowsMarketReferenceCurrency.toFixed()).toEqual('1788570685417553847');
    expect(userReserveTotals.totalCollateralMarketReferenceCurrency.toFixed()).toEqual('11819951652201573862');
    expect(userReserveTotals.currentLtv.toFixed()).toEqual('8000');
    expect(userReserveTotals.currentLiquidationThreshold.toFixed()).toEqual('8261.3698796199320993174');
  });
  ...
  it('should not compute collateral or LTV if usageAsCollateralEnabledOnUser is false', () => {
    ...
    expect(userReserveTotals.totalCollateralMarketReferenceCurrency.toFixed()).toEqual('0');
    expect(userReserveTotals.currentLtv.toFixed()).toEqual('0');
    expect(userReserveTotals.currentLiquidationThreshold.toFixed()).toEqual('0');
  });
});
