...
import { type BigNumberValue, normalize, valueToZDBigNumber } from './bignumber';
...
describe('ray math', () => {
  ...
  describe('rayPow and binomialApproximatedRayPow', () => {
    it('should be roughly equal', () => {
      const result = rayPow(
        valueToZDBigNumber('323788616402133497883602337').dividedBy(SECONDS_PER_YEAR).plus(RAY),
        valueToZDBigNumber(60 * 60 * 24)
      ).toString();
      const approx = binomialApproximatedRayPow(
        valueToZDBigNumber('323788616402133497883602337').dividedBy(SECONDS_PER_YEAR),
        valueToZDBigNumber(60 * 60 * 24)
      ).toString();
    ...
    `('should have close results for varying exponents', ({ exponents, errorLte }) => {
      const result = rayPow(
        valueToZDBigNumber('10000000000000000000000000').dividedBy(SECONDS_PER_YEAR).plus(RAY),
        valueToZDBigNumber(exponents)
      );
      const approx = binomialApproximatedRayPow(
        valueToZDBigNumber('10000000000000000000000000').dividedBy(SECONDS_PER_YEAR),
        valueToZDBigNumber(exponents)
      );

      const diff = result.gt(approx) ? result.minus(approx) : approx.minus(result);
      const diffPercentage = normalize(rayDiv(diff, result.multipliedBy(100)), 24).toString();
      expect(Math.abs(Number.parseFloat(diffPercentage))).toBeLessThanOrEqual(errorLte);
    });
    ...
    it.each`
      ...
      ${5}  | ${5}     | ${0.00003}
      ${5}  | ${10}    | ${0.0003}
    `('should not be far off for big amounts and time spans', ({ years, interest, errorLte }) => {
      /**
      ...
      const rate = valueToZDBigNumber(Number.parseFloat(interest) * 1000000000000000000000000);
      const balance = '100000000000000000000000000'; // 100M ETH
      const accurateInterest = legacyCalculateCompoundedInterest(rate, timeSpan, 0);
      const approximatedInterest = calculateCompoundedInterest(rate, timeSpan, 0);

      const accurateBalanceI = accurateInterest.multipliedBy(balance).minus(balance);
      const approximatedBalanceI = approximatedInterest.multipliedBy(balance).minus(balance);

      const diff = accurateBalanceI.minus(approximatedBalanceI);
      const diffPercentage = normalize(rayDiv(diff, accurateBalanceI.multipliedBy(100)), 24);
      expect(Math.abs(Number.parseFloat(diffPercentage))).toBeLessThanOrEqual(errorLte);
    });
  ...
  describe('rayDiv', () => {
    it('should divide ray', () => {
      const wad = '100000000000000000000000000';
      const ray = '100000000000000000000000000000000000';
      expect(rayDiv(ray, wad).toFixed()).toBe('1000000000000000000000000000000000000');
    });
  });
});
